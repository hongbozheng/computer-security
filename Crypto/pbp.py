...
def encrypt(rsakey, s):
    ...
    output += iv + cipher.encrypt(pad(s).encode("latin-1"))
    return encrypt_header + base64.b64encode(output).decode('ascii') + "\n" + encrypt_footer


# Decrypt a PBP message
# Parameters:
# rsakey: use RSA.construct() to build the RSA object
# c: the bytes from ciphertext file as a string
