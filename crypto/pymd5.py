...
class md5(object):
    ...
    def __init__(self, string='', state=None, count=0):
        ...
        if state is None:
            ...
            if len(state) == 32 and type(state) is str:
    ...
    def update(self, input):
        ...
        """
        import sys
        if sys.version_info[0] > 2 and type(input) is str:
            input = bytes(input, "latin-1")
        inputLen = len(input)
...
def _encode(input, length):
    k = length // 4
    mystery = ("%iI" % k,) + tuple(input[:k])
    res = struct.pack(*mystery)
    return res
